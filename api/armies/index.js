...
async function processArrival(mission, myKingdom, now) {
  ...
  if (mType === 'missile') {
    ...
    if (!targetKingdom) {
      // No real kingdom at target — missiles land with no effect
    ...
    // Trebuchet interception: each trebuchet stops 1 missile
    ...
    if (remaining > 0) {
      ...
      // Destroy defenses starting from cheapest (lowest hull armor)
      const defenseOrder = DEFENSE_KEYS.filter(k => k !== 'trebuchet')
      // Sort by armor per unit ascending
      defenseOrder.sort((a, b) => {
    ...
    // Notify defender (only real players)
    if (!targetKingdom.isNpc) {
      await db.insert(messages).values({
        userId: targetKingdom.userId,
        type: 'battle',
